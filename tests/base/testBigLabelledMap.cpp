...
bool
isEqual( Container1 const& c1, Container2 const& c2 )
{
  if ( c1.size() != c2.size() )
    return false;

  for ( typename Container1::const_iterator it1 = c1.begin(), it1_end = c1.end(); it1 != it1_end; ++it1 )
    {
      typename Container2::const_iterator const it2 = c2.find( it1->first );
      if ( it2 == c2.end() || it2->second != it1->second )
        return false;
    }
  ...
  for ( unsigned int i = 0; i < nb; ++i )
    {
      unsigned int idx = random() % ( l.max_size() );
      erase( v, l, idx );
      //std::cout << "  (" << i << "/" << nb << ") erase(" << idx << ") : l=" << l << std::endl; 
      is_equal &= isEqual( v, l );
    }
  return is_equal;
}
